...
async function scrapeCourseFromPopup(page, courseLink) {
  try {
    console.log('Attempting to click course link...');
    // Click the course link to open popup
    await courseLink.click();
    
    console.log('Waiting for popup content...');
    // Wait for popup content to load with increased timeout
    await page.waitForSelector('.td_dark .coursepadding', { timeout: 10000 });
    
    // Extract course information
    const courseInfo = await page.evaluate(() => {
      const container = document.querySelector('.td_dark .coursepadding');
      if (!container) {
        console.log('Could not find course container');
        return null;
      }
    ...
    console.log('Course info extracted:', courseInfo ? courseInfo.courseCode : 'null');

    // Click somewhere else to close the popup
    await page.click('body');
    await page.waitForTimeout(1000); // Increased delay to ensure popup closes
...
async function scrapeCoursePage(page, pageNum) {
  ...
  // Wait for course links to be visible
  console.log('Waiting for course links to load...');
  await page.waitForSelector('a.preview_td', { timeout: 10000 });
...
async function scrapeCourseCatalog(onPageScraped = null) {
  ...
  try {
    ...
    await page.waitForTimeout(5000); // Add extra wait time for dynamic content
